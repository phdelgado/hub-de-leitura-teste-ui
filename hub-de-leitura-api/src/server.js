...
app.get("/api/info", (req, res) => {
  res.json({
    ...
    author: "Fábio Araújo",
    contact: {
      email: "fabio.araujo.qa@gmail.com"
    },
    documentation: {
      swagger: "/api-docs",
...
// Página inicial - redireciona para API DOCS - Swagger
app.get("/", (req, res) => {
...
async function startServer() {
  try {
    ...
    const server = app.listen(port, () => {
      console.log("\n🚀 ===============================================");
      console.log("📚 API HUB DE LEITURA - Sistema de Biblioteca QA");
      console.log("🚀 ===============================================\n");
      
      const apiUrl = `http://localhost:${port}/api-docs`;
      console.log(`📖 Documentação API: ${apiUrl}`);
      ...
      // Abrir navegador automaticamente após 2 segundos (apenas em desenvolvimento)
      if (process.env.NODE_ENV !== "production") {
        setTimeout(() => {
          openBrowser(apiUrl);
        }, 2000);
    ...
    server.on('error', (error) => {
      if (error.code === 'EADDRINUSE') {
        ...
        console.log(`💡 Tente usar uma porta diferente: PORT=3001 npm start`);
        process.exit(1);
      } else {
